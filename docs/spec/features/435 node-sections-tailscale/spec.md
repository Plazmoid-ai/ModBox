...
- ключ узла: одиночный сервер — `id` источника, член папки и узел подписки —
  `id` контейнера + сырой тег (адрес NodeLink), тёзка — `#2`;
- индекс `<filesDir>/tailscale_state.json` «слот Workspaces → ключ → имя
  каталога». Имя выдаётся один раз (финальная форма, при занятости `-1`…) и не
  меняется: переименование, перенос и смена префикса переписывают только ключ
  записи, каталог открытого ядром узла не двигается;
- удаление узла, члена, папки или источника снимает запись сразу, каталог
  удаляется, если на него не ссылается другой слот и ядро остановлено; иначе —
  при ближайшей сборке с остановленным ядром (п. 3, с info-строкой). Ожидаемый
  набор — все хранимые узлы, включая выключенные;
- миграция: первая сборка слота подхватывает каталог 2.24.0 по финальной
  форме, файлы не двигаются;
- Workspaces: у каждого слота свой набор записей, каталоги не копируются;
  Save as копирует записи (узел-копия — то же устройство), Delete слота удаляет
  каталоги без других ссылок, Rename переносит набор;
- п. 4: индекс и каталоги вне `lxbox_settings.json`, ни во внутренний бэкап,
  ни в LX Backup не попадают.

Имя каталога у LxBox больше не равно санитизированному финальному тегу для
узлов, переименованных после первой сборки; в конфиг уходит только путь.
